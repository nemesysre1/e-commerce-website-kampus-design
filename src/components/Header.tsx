        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            {["beranda", "promo", "produk", "tentang"].map((sec) => (
              <a key={sec} href={`#${sec}`} onClick={(e)=>{e.preventDefault(); scrollToSection(sec);}} className="text-foreground hover:text-primary transition-colors cursor-pointer">
          ...
          <div className="flex items-center space-x-4">
            {!user && (
              <Button variant="ghost" size="sm" onClick={onLoginClick} className="hidden md:flex items-center space-x-2">
            ...
            <Button variant="ghost" size="sm" onClick={onCartClick} className="relative">
              ...
              {cartItemCount > 0 && (
                <Badge className="absolute -top-2 -right-2 min-w-5 h-5 flex items-center justify-center p-0 text-xs">{cartItemCount}</Badge>
              )}
            </Button>
